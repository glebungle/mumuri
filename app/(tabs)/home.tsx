    ...
    <Modal visible={visible} transparent animationType="fade">
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          <View style={styles.modalIconBox}><Ionicons name="information-circle" size={32} color="#6198FF" /></View>
          <AppText style={styles.modalTitle}>알림</AppText>
          <AppText type="medium" style={styles.modalMessage}>{message}</AppText>
          <Pressable style={styles.modalButton} onPress={onClose}><AppText style={styles.modalButtonText}>확인</AppText></Pressable>
        </View>
      </View>
    </Modal>
  );
...
export default function HomeScreen() {
  ...
  const switchTab = (targetIndex: 0 | 1) => {
    ...
    Animated.timing(tabAnim, { toValue: targetIndex, duration: 300, useNativeDriver: false }).start();
  ...
  const indicatorTranslateX = tabAnim.interpolate({ inputRange: [0, 1], outputRange: [0, 51] });
  const indicatorWidth = tabAnim.interpolate({ inputRange: [0, 1], outputRange: [36, 48] });
  const headerTintColor = tabAnim.interpolate({ inputRange: [0, 1], outputRange: ['#FFF', '#000'] });
  const homeTextColor = tabAnim.interpolate({ inputRange: [0, 1], outputRange: ['rgba(255,255,255,1)', 'rgba(0,0,0,0.3)'] });
  const galleryTextColor = tabAnim.interpolate({ inputRange: [0, 1], outputRange: ['rgba(255,255,255,0.5)', 'rgba(0,0,0,1)'] });
  const dDayOpacity = tabAnim.interpolate({ inputRange: [0, 1], outputRange: [1, 0] });
          ...
          <View style={styles.tabSwitchContainer}>
            <View style={styles.tabButtons}>
              <Pressable onPress={() => switchTab(0)} style={styles.tabBtn}>
                <Animated.Text style={[styles.tabText, { color: homeTextColor, fontFamily: activeTab === 0 ? 'Pretendard-Bold' : 'Pretendard-Medium' }]}>홈</Animated.Text>
              </Pressable>
              <Pressable onPress={() => switchTab(1)} style={styles.tabBtn}>
                <Animated.Text style={[styles.tabText, { color: galleryTextColor, fontFamily: activeTab === 1 ? 'Pretendard-Bold' : 'Pretendard-Medium' }]}>갤러리</Animated.Text>
              </Pressable>
            </View>
          ...
          <View style={{ flex: 1 }}>
            <View style={styles.backgroundLayer}>
              ...
              >
                ...
                <LinearGradient colors={['transparent', '#FFFCF5']} style={styles.gradientOverlay} locations={[0.2, 1]} />
              </ImageBackground>
            </View>
            ...
            <View style={styles.homeContentContainer}>
              ...
              <View style={[styles.dashboard, { paddingBottom: insets.bottom }]}>
                ...
                <View style={styles.bottomRow}>
                  <Pressable style={({ pressed }) => [styles.squareCard, styles.calendarCard, pressed && styles.pressedCard, !isCoupled && styles.disabledCard]} onPress={handlePressCalendar}>
                  ...
                  <Pressable style={({ pressed }) => [styles.squareCard, styles.chatCard, pressed && styles.pressedCard, !isCoupled && styles.disabledCard]} onPress={handlePressChat}>
