...
type Schedule = { id: number; title: string; startAt: string; endAt: string; allDay: boolean; couple: boolean; ownerType: 'ME' | 'PARTNER'; };
    ...
    return (
      <Pressable
        style={[
          ...
          isSelected && (mode === 'SCHEDULE' ? styles.dayCellSelectedSchedule : styles.dayCellSelectedBorder)
        ]}
      ...
      <View style={styles.modalOverlay}>
        <Animated.View style={[styles.modalContent, { transform: [{ translateY: panY }] }]} {...panResponder.panHandlers}>
          ...
          <View style={styles.titleInputRow}>
             ...
             <TextInput style={[styles.modalTitleInput, {fontFamily:'Pretendard-Bold'}]} placeholder="제목" placeholderTextColor="#999" value={title} onChangeText={setTitle} />
          </View>
          <View style={styles.timeSection}>
                ...
                <View style={styles.timeInputContainer}>
                    ...
                    {!isAllDay && (
                        <View style={styles.timeInputBox}>
                            <TextInput style={styles.timeInput} keyboardType="number-pad" value={startHour} onChangeText={setStartHour} maxLength={2} />
                            <AppText type='semibold' style={styles.timeColon}>:</AppText>
                            <TextInput style={styles.timeInput} keyboardType="number-pad" value={startMin} onChangeText={setStartMin} maxLength={2} />
                        </View>
                    )}
                </View>
             </View>
             ...
             <View style={styles.timeRow}>
                ...
                <View style={styles.timeInputContainer}>
                    ...
                    {!isAllDay && (
                        <View style={styles.timeInputBox}>
                            <TextInput style={styles.timeInput} keyboardType="number-pad" value={endHour} onChangeText={setEndHour} maxLength={2} />
                            <AppText type='semibold' style={styles.timeColon}>:</AppText>
                            <TextInput style={styles.timeInput} keyboardType="number-pad" value={endMin} onChangeText={setEndMin} maxLength={2} />
                        </View>
                    )}
                </View>
             </View>
...
export default function CalendarScreen() {
  ...
  const bgColor = modeAnim.interpolate({ inputRange: [0, 1], outputRange: ['#FFFCF5', '#1C1C1E'] });
  const headerTextColor = modeAnim.interpolate({ inputRange: [0, 1], outputRange: ['#111111', '#FFFFFF'] });
  const switchBgColor = modeAnim.interpolate({ inputRange: [0, 1], outputRange: ['#111111', '#FFFCF5'] });
  const switchTextColor = modeAnim.interpolate({ inputRange: [0, 1], outputRange: ['#999999', '#999999'] });

  const fetchMissions = useCallback(async (targetMonth: string) => {
    try {
      const token = await AsyncStorage.getItem('token');
      const dateObj = parseISO(targetMonth);
      const res = await fetch(`${BASE_URL}/calendar/missions?year=${dateObj.getFullYear()}&month=${dateObj.getMonth() + 1}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const fetchSchedules = useCallback(async (targetMonth: string) => {
    try {
      ...
      const res = await fetch(`${BASE_URL}/calendar/schedules?year=${date.getFullYear()}&month=${date.getMonth() + 1}`, { headers: { Authorization: `Bearer ${token}` } });
      const data = res.ok ? await res.json() : [];
  ...
  const handleAddSchedule = async (input: any) => {
    try {
        ...
        const startTimeStr = input.isAllDay ? "00:00:00" : `${pad(input.start.hour)}:${pad(input.start.minute)}:00`;
        const endTimeStr = input.isAllDay ? "23:59:59" : `${pad(input.end.hour)}:${pad(input.end.minute)}:00`;

        const body = {
            title: input.title, date: selectedDate,
            startTime: startTimeStr, endTime: endTimeStr,     
            allDay: input.isAllDay, couple: input.isCouple
        };
  ...
  useEffect(() => { fetchMissions(currentMonth); fetchSchedules(currentMonth); }, [currentMonth]);

  useEffect(() => {
    if (calendarMode === 'MISSION') setSelectedPhotos(photosByDate[selectedDate] || []);
    else setSelectedSchedules(schedulesByDate[selectedDate] || []);
  }, [selectedDate, calendarMode, photosByDate, schedulesByDate]);

  if (loading) return <View style={[styles.center, { backgroundColor: calendarMode === 'SCHEDULE' ? '#1C1C1E' : '#FFFCF5' }]}><ActivityIndicator size="large" color="#999" /></View>;
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          <Pressable onPress={() => router.back()} style={styles.backBtn}>
            <Animated.Text style={{ color: headerTextColor }}><Ionicons name="chevron-back" size={28} /></Animated.Text>
          </Pressable>
        ...
        <Pressable onPress={() => {
          ...
          Animated.timing(modeAnim, { toValue: next === 'SCHEDULE' ? 1 : 0, duration: 300, useNativeDriver: false }).start();
        }}>
      ...
      <View style={styles.monthNavRow}>
        <View style={styles.monthNavControls}>
          <Pressable onPress={() => setCurrentMonth(format(subMonths(parseISO(currentMonth), 1), 'yyyy-MM-01'))} style={styles.monthNavBtn}>
            <Animated.Text style={{ color: headerTextColor }}><Ionicons name="chevron-back" size={20} /></Animated.Text>
          </Pressable>
          ...
          <Pressable onPress={() => setCurrentMonth(format(addMonths(parseISO(currentMonth), 1), 'yyyy-MM-01'))} style={styles.monthNavBtn}>
      ...
      <View style={styles.CalenderContainer}>
        <Calendar
          ...
          theme={{
            ...
            todayTextColor: '#6198FF', dayTextColor: '#888', textDisabledColor: '#555',
      ...
      <View style={[styles.bottomContainer, { paddingBottom: Math.max(insets.bottom, 16) }]}>
        {calendarMode === 'MISSION' ? (
          selectedPhotos.length === 0 ? (
            <View style={styles.emptyBox}><AppText style={styles.emptyText}>이 날의 미션 기록이 없어요.</AppText></View>
          ) : (
            <FlatList
              data={selectedPhotos} keyExtractor={(item) => item.id} horizontal pagingEnabled
              showsHorizontalScrollIndicator={false}
              renderItem={({ item }) => {
                const avatar = (item.ownerType === 'ME' ? userData?.myProfileImageUrl : userData?.partnerProfileImageUrl) 
                              ? { uri: item.ownerType === 'ME' ? userData?.myProfileImageUrl : userData?.partnerProfileImageUrl } 
                              : defaultProfileImg;
                return (
                  <View style={styles.previewCard}>
                    <ImageBackground source={{ uri: item.url }} style={styles.previewImage} resizeMode="cover">
                      <View style={styles.previewHeaderOverlay}>
                        <View style={styles.previewAvatar}><Image source={avatar} style={{ width: '100%', height: '100%' }} /></View>
                        <View>
                          <AppText type='pretendard-b' style={styles.previewNameText}>{item.ownerNickname}</AppText>
                      ...
                      {item.missionTitle && (
                        <View style={styles.previewMissionBadge}><AppText style={styles.previewMissionText}>{item.missionTitle}</AppText></View>
                      )}
                    </ImageBackground>
                  </View>
                );
          ...
          <>
            ...
            <FlatList 
              ...
              renderItem={({item}) => {
                const barColor = item.couple ? '#FF8080' : (item.ownerType === 'ME' ? '#6198FF' : '#4CD964');
                const isPartnerOnly = item.ownerType === 'PARTNER' && !item.couple;
                return (
                  <View style={[styles.scheduleItem, { backgroundColor: barColor }]}>
                    <View style={styles.scheduleTimeBox}><AppText type="bold" style={styles.scheduleTimeText}>{item.allDay ? '하루 종일' : format(parseISO(item.startAt), 'HH:mm')}</AppText></View>
              ...
              ListEmptyComponent={<View style={styles.emptyBox}><AppText style={styles.emptyText}>일정이 없습니다.</AppText></View>}
            />
          </>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  photoSelectedOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(97, 152, 255, 0.2)' },
  photoDateOverlay: { position: 'absolute', top: 1, width: '100%', alignItems: 'center' },
  ...
  previewCard: { width: SCREEN_WIDTH - 32, flex: 1, maxHeight: SCREEN_HEIGHT * 0.42, borderRadius: 24, overflow: 'hidden', backgroundColor: '#000' },
  previewImage: { width: '100%', height: '100%' },
  previewHeaderOverlay: { flexDirection: 'row', alignItems: 'center', padding: 16, gap: 10 },
  ...
  previewMissionBadge: { position: 'absolute', bottom: 16, left: 16, backgroundColor: 'rgba(0,0,0,0.5)', paddingVertical: 8, paddingHorizontal: 12, borderRadius: 12 },
  previewMissionText: { color: '#FFF', fontSize: 11 },
  scheduleHeader: { marginBottom: 10 },
  scheduleItem: { flexDirection: 'row', alignItems: 'center', borderRadius: 12, padding: 12, marginBottom: 8, height: SCREEN_HEIGHT*0.07 },
  scheduleTimeBox: { width: 65, alignItems: 'center' },
  scheduleTimeText: { color: '#FFF', fontSize: 12 },
  verticalBar: { width: 1, height: 16, backgroundColor: 'rgba(255,255,255,0.4)', marginRight: 12 },
  ...
  timeConnector: { width: 1, height: 15, backgroundColor: '#555', marginLeft: 9 },
