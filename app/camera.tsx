...
export default function CameraHome() {
  ...
  const takePhoto = async () => {
    ...
    try {
      ...
      const currentW = viewW || screenWidth;
      const currentH = viewH || CAMERA_HEIGHT;
  ...
  const pickFromGallery = async () => {
    try {
      const result = await ImagePicker.launchImageLibraryAsync({
        // 수정 전: mediaTypes: ImagePicker.MediaTypeOptions.Images,
        // 수정 후: MediaTypeOptions -> MediaType 으로 변경
        mediaTypes: ImagePicker.MediaTypeOptions.Images, 
        allowsEditing: true, 
        aspect: [screenWidth, Math.round(screenHeight * 0.8)], 
        quality: 1,
      });
  ...
  return (
    <View style={styles.fullScreenContainer}>
      ...
      <SafeAreaView style={[styles.uiOverlay, { paddingTop: insets.top }]}>
        {previewUri && previewUri !== 'PENDING_MERGE' && <View style={styles.topBarPreview}><Pressable onPress={retake} style={styles.topIconBtnRetake}><Ionicons name="close" size={24} color="#fff" /></Pressable></View>}
        {previewUri === 'PENDING_MERGE' && <View style={styles.topBarPreview}><Pressable onPress={retake} style={styles.topIconBtnRetake}><Ionicons name="close" size={24} color="#fff" /></Pressable></View>}
                ...
                <ViewShot ref={viewShotRef} options={{ format: 'jpg', quality: 0.9 }} style={StyleSheet.absoluteFill}>
                    <View style={styles.gridContainer}>
                        {[0, 1, 2, 3].map((index) => {
                            ...
                            // photoUri가 있으면 이미지, 없으면 투명(active) 또는 검정(inactive)
                            // 카메라가 이미 해당 위치에 와있으므로 'active' 상태는 투명하게 두면 됨
                            const isActive = index === layoutPhotos.length;
                            
                            return (
                                <View key={index} style={[styles.gridImage, { 
                                    backgroundColor: photoUri ? 'transparent' : (isActive ? 'transparent' : 'black') 
                                }]}>
                                    {photoUri && (
...
const styles = StyleSheet.create({
  ...
  cameraFrame: { width: screenWidth, height: screenHeight * 0.8, borderRadius: 0, overflow: 'hidden', marginTop: 0,  backgroundColor: 'black', },
  ...
  topHeaderContainer: { position: 'absolute', top: screenHeight*0.09, left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, zIndex: 10 },
  ...
  hintBubbleWrap: { position: 'absolute', top: screenHeight * 0.16, width: '100%', alignItems: 'center', paddingHorizontal: screenWidth*0.1 },
  ...
  hintBubble: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 4, paddingVertical: 12, zIndex: 15, backgroundColor: 'rgba(255,255,255,0.7)', borderRadius: 12, maxWidth: '85%' },
  innerArrowArea: { width: 32, alignItems: 'center', justifyContent: 'center' },
  hintTextWrap: { flex: 1, alignItems: 'center', justifyContent: 'center' ,},
  hintText: { color: '#444444', fontSize: 11, textAlign: 'center'},
  bottomOverlay: { position: 'absolute', left: 0, right: 0, bottom: 0, alignItems: 'center', justifyContent: 'center', zIndex: 50 },
  bottomButtonsRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', width: '100%', paddingHorizontal: 40, marginBottom:screenHeight*0.1, },
