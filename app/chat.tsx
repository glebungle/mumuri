...
const ChatText = (props: React.ComponentProps<typeof AppText>) => {
  ...
  return (
    <AppText {...rest} style={[{ fontFamily: "Pretendard-Medium" }, style]} />
  );
};
...
export default function ChatScreen() {
    ...
    (p: ChatIncoming) => {
      setMessages((prev) => {
        ...
        const add: ChatMessage = {
          ...
          blurred: (p as any).blurred ?? false,
          blurMessage: (p as any).blurMessage || "",
        };
      ...
      const init = async () => {
        ...
        try {
          ...
          const historyMsgs = rows.map((r: any) => ({
            ...
            blurred: r.blurred ?? false,
            blurMessage: r.blurMessage || "",
          }));
            ...
            handlers: {
              ...
              onConnected: () => {
                chatRef.current?.markAsRead(
                  ROOM_KEY,
                  Number(userId),
                  latestVisibleMsgId.current ?? undefined,
                );
              },
            },
          });
  ...
  const renderItem: ListRenderItem<ChatMessage | DateMarker> = useCallback(
    ({ item, index }) => {
      ...
      const showTime =
        !nextMsg ||
        isDateMarker(nextMsg) ||
        (nextMsg as ChatMessage).mine !== m.mine ||
        !sameMinute((nextMsg as ChatMessage).createdAt, m.createdAt);
      const bubbleStyle = (
        m.type === "mission_text"
          ? m.mine
            ? styles.bubbleMissionMine
            : styles.bubbleMissionOther
          : m.mine
            ? styles.bubbleMine
            : styles.bubbleOther
      ) as ViewStyle;
          ...
          {!m.mine && (
            <View style={styles.profileContainer}>
              {m.isFirstInGroup ? (
                <Pressable
                  onPress={() =>
                    openImageViewer(userData?.partnerProfileImageUrl || null)
                  }
                >
                  <Image
            ...
            >
              ...
              >
                {m.type === "mission_text" ? (
                  ...
                  <Pressable
                    onPress={() => !m.blurred && openImageViewer(m.imageUrl!)}
                  ...
                  >
                    <Image
                      ...
                      resizeMode="cover"
                      blurRadius={m.blurred ? 20 : 0} //블러 처리
                    />
                    {m.blurred && (
                      <View style={styles.imageOverlay}>
                        ...
                        <Pressable
                          style={styles.goCameraBtn}
                          onPress={() => router.push("/camera")}
                        >
                          <AppText
                            type="pretendard-b"
                            style={styles.goCameraBtnText}
                          >
                            사진 찍으러 가기
                          </AppText>
                        </Pressable>
      ...
      >
        <View style={{ flex: 1 }}>
          ...
          <View
            style={[
              styles.inputBar,
              { paddingBottom: isIOS ? iosBottomPadding : androidPadding },
            ]}
          >
            <Pressable
              onPress={() => router.push("/camera")}
...
const styles = StyleSheet.create({
  ...
  goCameraBtnText: { color: "#000", fontSize: 12 },
